...
module.exports.run = async (bot, message, args, ops) => {
    ...
    if (!message.member.voiceChannel) return message.channel.send("Verbind met een kanaal");

    // if (message.guild.me.voiceChannel) return message.channel.send("Sorry de bot is al geconecteerd met een spraak kanaal");

    if (!args[0]) return message.channel.send("Geef een URL op.");
    ...
    if (!data.dispatcher) {
        ...
        message.channel.send(`Toegevoegd aan de wachtrij: ${info.title} | Gevraagd door: ${message.author.tag}`);
